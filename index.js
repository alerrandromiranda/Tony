...
limitt = 50
ban = []
userpremium = ["+5511957505042@s.whatsapp.net"] //ubah nomer kalian
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo (a) ao grupo☕ *${mdata.subject}*` por favor não seja um ghost
				...
				teks = `Tchau e não volte @${num.split('@')[0]}Ghost do caralho👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
