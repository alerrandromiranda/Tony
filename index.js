...
rmenu = "Olá amigos do Tony"
limitt = 50
ban = []
userpremium = ["554398434473@s.whatsapp.net"] //ubah nomer kalian
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Adeus  @${num.split('@')[0]} Já vai tarde filho da Puta👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
...
const getRegisteredRandomId = () => {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const FarhanGans = ["554398434473@s.whatsapp.net"] // apenas mude esta tudo bem
			...
			mess = {
				...
				only: {
					...
					userB: `Olá*${pushname2}* Você ainda não se registrou  *${name}* \nPara se registrar envie o comando \n\n exemplo ${prefix} daftar ${pushname2}/17/brasil`,
			...
			const ownerNumber = ["554398434473@s.whatsapp.net"] // owner number ubah aja
		...
		case 'daftar':
			...
			if (isBanned) return reply(mess.only.benned)
				...
				ben = '```Lista dos Banidos``` :\n'
					...
					break 
				case 'premiumlist':
				prem = '```Lista Users Premium  TONY B O T``` :\n'
           ...
           const imgbb = require('imgbb-uploader')
            ...
            break  
				...
				case 'addprem':
					...
					reply(`*PARABÉNS ${addpremium} AGORA VOCÊ É UM USUÁRIO PREMIUM*\n\nDIGITE _MENU PARA AS OPÇÕES DE PREMIUM:)`)
					break
				case 'removeprem':
					if (!isOwner) return reply(mess.only.ownerB)
					rprem = body.slice(13)
				...
				case 'block':
				 ...
				 client.chatRead (from)
					...
					client.sendMessage(from, `O alvo foi bloqueado ${body.slice(7)}@c.us`, text)
					break
				case 'unblock':
                    if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, `O alvo foi desbloqueado ${body.slice(9)}@c.us`, text)
				    break
         ...
         case 'lirik':
                ...
                if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('554398434473@s.whatsapp.net', options, text, {quoted: mek})
				...
				case 'nsfwloli':
				    ...
				    if (isLimit(sender)) return reply(limitend(pushname2))
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Loli? isso vai da ruim mano'})
					} catch (e) {
			    ...
			    case 'nsfwblowjob':
				    ...
				    if (isLimit(sender)) return reply(limitend(pushname2))
						...
						buffer = await getBuffer(res.result)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Por Favor Não se masturbe com essas imagens'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
			    ...
			    case 'nsfwneko':
				    ...
				    if (isLimit(sender)) return reply(limitend(pushname2))
						...
						buffer = await getBuffer(res.result)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ta ai seu tarado'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
				...
				case 'nsfwtrap':
				    ...
				    if (isLimit(sender)) return reply(limitend(pushname2))
						...
						buffer = await getBuffer(res.result)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'TA AI PUNHETEIRO'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
				...
				case 'hentai':
				    ...
				    if (isLimit(sender)) return reply(limitend(pushname2))
						...
						buffer = await getBuffer(res.result)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ta ai Safado (a)'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
...
// only grup fitur anime
              case 'anime':
                ...
                if (!isAnime) return reply(' *Harus Mengaktifkan Mode Anime* ')
					...
					client.sendMessage(from, pok, image, { quoted: mek , caption: 'Ta Ai Mano'})
					await limitAdd(sender) 
					break  
				case 'animekiss':
                if (isBanned) return reply(mess.only.benned)    
...
//akhir kreator
		    ...
		    case 'map':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔═══✪ Marcando Todos os membros*${pushname2}* ✪══`+ teks +'╚═══〘 TONY BOT 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
			    ...
			    reply(mess.wait)
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args[0] === 'buka') {
					    reply(`\`\`\`✓O Grupo foi aberto com sucesso\`\`\` *${groupMetadata.subject}*`)
						client.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'tutup') {
						reply(`\`\`\`O Grupo foi fechado\`\`\` *${groupMetadata.subject}*`)
				...
				case 'clearall':
					...
					reply(`\`\`\`Exclua todos os chats com sucesso TONY BOT\`\`\``)
					break
                                case 'bcgc':
					client.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					if (mentioned.length > 1) {
						...
						mentions(`Pedidos recebidos, emitidos, removendo (a) por não seguir as regras do grupo. : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'edotense':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque o membro que você quer dar adm')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						...
						mentions(`Parabéns, @${mentioned[0].split('@')[0]} Agora você é um (a) admin do grupo *${groupMetadata.subject}*`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				case 'delete':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque o membro que você quer tirar da lista de admins')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						...
						mentions(`Pedidos recebidos, Admin rebaixado : @${mentioned[0].split('@')[0]} Agora é um membro comum`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'listadmins':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					if ((args[0]) === 'on') {
						...
						reply(`\`\`\`✓Ativo com sucesso o modo nsfw no grupo\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/json/nsfw.json', JSON.stringify(nsfw))
						reply(`\`\`\`Modo nsfw desativado no grupo \`\`\` *${groupMetadata.subject}*`)
					} else {
						reply('On ativar, Off desabilitar')
					}
					break
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if ((args[0]) === 'on') {
						...
						reply(`\`\`\`✓Ativo com sucesso o modo anime no grupo\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						anime.splice(from, 1)
						fs.writeFileSync('./database/json/anime.json', JSON.stringify(anime))
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (args.length < 1) return reply('Hmmmm')
					if ((args[0]) === 'on') {
						if (isWelkom) return reply('Este Comando já está ativo')
						welkom.push(from)
				...
				if (!isUser) return reply(mess.only.userB)
					...
					try {
						...
						mentions(` Foto do perfil atualizada com sucesso usando a foto do perfil do (a) @${id.split('@')[0]}`, [jid], true)
					} catch (e) {
						reply('Om falha')
					}
					break
				...
				case 'setpref':
					...
					if (!isOwner) return reply(mess.only.ownerB)
					prefix = args[0]
					reply(`O Prefixo foi mudado para : ${prefix}`)
					break 
				case 'setmemlimit':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
