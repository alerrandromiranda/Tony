...
limitt = 50
ban = []
userpremium = ["+554384758863@s.whatsapp.net"] //ubah nomer kalian
...
const getRegisteredRandomId = () => {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const FarhanGans = ["+554384758863@s.whatsapp.net"] // apenas mude esta tudo bem
			...
			const ownerNumber = ["+5543984758863@s.whatsapp.net"] // owner number ubah aja
		...
		case 'daftar':
         ...
         case 'lirik':
                ...
                if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('+554384758863@s.whatsapp.net', options, text, {quoted: mek})
                ...
                if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('+5543984758863@s.whatsapp.net', options, text, {quoted: mek})
...
//akhir kreator
		    ...
		    case 'map':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔═══✪ Marcando Os Ghosts*${pushname2}* ✪══`+ teks +'╚═══〘 TONY BOT 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
